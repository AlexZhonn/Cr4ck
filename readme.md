...
- **Learning Paths** — 8 curated sequences (OOP Foundations → Advanced OOP, Creational/Structural/Behavioral Patterns, System Design Foundations, Distributed Systems Deep Dive) with per-challenge progress tracking
...
- **Badges / Achievements** — 15 badges awarded automatically on milestones (first solve, perfect score, streak goals, XP milestones, volume milestones); badge shelf on profile, pop-up toast on earn
...
| `ANTHROPIC_API_KEY` | Yes* | Server-level Claude key. *Not needed if `ALLOW_SERVER_KEY=false`. Also used by the daily challenge generator. |
...
| `ADMIN_SECRET` | Yes (prod) | Secret for `X-Admin-Secret` header; gates `POST /api/admin/generate-daily` |
...
| `013_test_harness_and_starter_codes.sql` | Adds `test_harness` (hidden driver) + `starter_codes` JSONB (multi-language) columns |
| `013b_harness_data.sql` | Populates `test_harness` + updates `test_cases` for all 300 challenges (generated by `api/scripts/generate_harnesses.py`) |
| `014_daily_challenges.sql` | Adds `is_ai_generated` + `generated_at` to `challenges`; creates `daily_challenges` table (one row per date) |
| `015_badges.sql` | Creates `badges` catalog + `user_badges` join table; seeds 15 badges |
| `016_learning_paths.sql` | Creates `paths` + `path_challenges` tables; seeds 8 curated learning paths |
...
| Method | Path | Auth | Description |
| --- | --- | --- | --- |
| GET | `/api/v1/daily` | — | Today's challenge (Redis-cached until midnight UTC); 404 if not yet generated |
| POST | `/api/admin/generate-daily` | X-Admin-Secret | Generate today's challenge via Claude; falls back to random challenge on failure |

### Badges

| Method | Path | Auth | Description |
| --- | --- | --- | --- |
| GET | `/api/v1/badges` | — | Full badge catalog (15 badges) |
| GET | `/api/v1/badges/me` | Required | Badges earned by the current user |

### Learning Paths

| Method | Path | Auth | Description |
| --- | --- | --- | --- |
| GET | `/api/v1/paths` | — | All 8 curated learning paths with challenge counts and difficulty tags |
| GET | `/api/v1/paths/:slug` | — | Path detail with ordered challenges list |
| GET | `/api/v1/paths/:slug/progress` | Required | User's completion status for each challenge in the path |
...
│       ├── Login/
│       ├── Problem/
│       ├── PathDetail/         # /paths/:slug — sequential challenge list + progress bar
│       ├── Paths/              # /paths — grid of learning path cards
│       ├── ProblemSet/
│       ├── Profile/
│       ├── Register/
│       ├── ResetPassword/
...
│   └── daily-challenge.yml     # AI-generated daily challenge (midnight UTC cron)
│
└── Makefile
```
