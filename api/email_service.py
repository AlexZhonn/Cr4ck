...
def send_password_reset_email(to_email: str, username: str, token: str) -> None:
    ...
    if not POSTMARK_API_KEY:
        logger.warning("[email] POSTMARK_API_KEY not set — skipping password reset email")
        return
    ...
    html_body = f"""
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
      <h2 style="color:#7c3aed">Reset your Cr4ck password</h2>
      <p>Hey {username},</p>
    ...
    text_body = (
        f"Hey {username},\n\n"
        f"Reset your Cr4ck password by visiting:\n{reset_url}\n\n"
    ...
    try:
        resp = httpx.post(
            ...
            json={
                "From": POSTMARK_FROM,
                "To": to_email,
                "Subject": "Reset your Cr4ck password",
                "HtmlBody": html_body,
                "TextBody": text_body,
                "MessageStream": "outbound",
            },
        ...
        logger.error("[email] Failed to send password reset email: %s", exc)
